function [output, Greg] = dftregistration_cap(buf1ft, buf2ft, usfac, maxshift)
% function [output Greg] = dftregistration_cap(buf1ft,buf2ft,usfac,maxshift);
...
if nargin < 4, maxshift = size(buf1ft) / 2; end
if numel(maxshift) == 1, maxshift(2) = maxshift(1); end
maxshift(1) = min(size(buf1ft, 1)/2, maxshift(1));
maxshift(2) = min(size(buf1ft, 2)/2, maxshift(2));
...
elseif usfac == 1,
    ...
    %     [max1,loc1] = max(CC);
    range1 = unique([1:1 + floor(maxshift(1)), 1 + m - floor(maxshift(1)):m]);
    range2 = unique([1:1 + floor(maxshift(2)), 1 + n - floor(maxshift(2)):n]);
    [max1, loc1] = max(CC(range1, range2));
    [max2, loc2] = max(max1);
    rloc = range1(loc1(loc2));
    cloc = range2(loc2);
    CCmax = CC(rloc, cloc);
    ...
    range1 = unique([1:1 + floor(2*maxshift(1)), 1 + mlarge - floor(2*maxshift(1)):mlarge]);
    range2 = unique([1:1 + floor(2*maxshift(2)), 1 + nlarge - floor(2*maxshift(2)):nlarge]);
    [max1, loc1] = max(CC(range1, range2));
    [max2, loc2] = max(max1);
    rloc = range1(loc1(loc2));
    cloc = range2(loc2);
...
[nr, nc] = size(in);
% Set defaults
if nargin < 6, coff = 0; end
if nargin < 5, roff = 0; end
if nargin < 4, usfac = 1; end
if nargin < 3, noc = nc; end
if nargin < 2, nor = nr; end

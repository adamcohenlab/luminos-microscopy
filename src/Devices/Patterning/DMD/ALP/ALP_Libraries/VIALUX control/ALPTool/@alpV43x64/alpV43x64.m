classdef alpV43x64 < alpapi
    %alpV43x64 provides access to the ALP Highspeed API, v4.2, 64bit
    %
    %   Class alpV43x64 is derived from the abstract class alpapi and
    ...
    methods
        % constructor
        function obj = alpV43x64(tag, pseudoDLL, logger)
            ...
            if nargin >= 2 && pseudoDLL
                ...
                if isempty(alplibs)
                    ...
                    error('alpV43x64:dllnotinstalled', 'ALP v4 64 bit: Specified DLL is not installed');
                end
                if ~strcmp(alplibs.arch, computer('arch'))
                    % architecture mismatch
                    error('alpV43x64:architecturemismatch', 'ALP v4 64 bit: Architecture mismatch');
                end
                if isempty(alplibs.classname)
                    % DLL not registered in alplibs.m
                    error('alpV43x64:notregistered', 'ALP v4 64 bit: Not registered in alplibs.m');
                end
                if isempty(alplibs.dllfile) || ~exist(alplibs.dllfile, 'file')
                    % DLL is not available on this system
                    error('alpV43x64:nodll', 'ALP v4 64 bit: DLL is not available');
                end
                if isempty(alplibs.protofile) || ~exist(alplibs.protofile, 'file')
                    % prototype file is not available on this system
                    error('alpV43x64:noproto', 'ALP v4 64 bit: Prototype file is not available');
                end

                obj.libalias = 'alpV43x64';

% Newport TRB25CC
classdef Newport_Motor_Controller < Linear1D_Controller
    ...
    methods
        ...
        function obj = Newport_Motor_Controller(Initializers)
            obj = obj@Linear1D_Controller(Initializers);
            ...
            if flag == -1, error('get backlash correction (BA_Get) error');end
             obj.homeZ(); % Single home command sometimes gets ignored
             obj.motorAPI.PA_Set(1,0); % Initialize in home positon = at highest position, 0mm
...
%         end
        ...
        function success = Move_To_Position(obj, nextPositionStruct)
            ...
            if nextPosition < obj.pos.zAbs 
                
                backlashCorPosition = max(nextPosition - obj.backlashCor,obj.lowerLim); % backlash correction
                
                flag = obj.motorAPI.PA_Set(1,backlashCorPosition);
                if flag == -1, error('set position (PA_Set) error'); success = -1;end
                while obj.isMoving;pause(.001);end % wait for move to complete. 

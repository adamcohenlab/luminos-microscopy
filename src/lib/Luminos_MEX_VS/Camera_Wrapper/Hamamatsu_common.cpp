...
void dcamcon_show_dcamdev_info(HDCAM hdcam) {
  ...
  if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_MODEL, model, sizeof(model))) {
    dcamcon_show_dcamerr(hdcam, err, "dcamdev_getstring(DCAM_IDSTR_MODEL)\n",
                         NULL);
  } else if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_CAMERAID, cameraid,
                                sizeof(cameraid))) {
...
void dcamcon_show_dcamdev_info_detail(HDCAM hdcam) {
  ...
  if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_CAMERAVERSION, buf,
                         sizeof(buf)))
    dcamcon_show_dcamerr(hdcam, err,
                         "dcamdev_getstring(DCAM_IDSTR_CAMERAVERSION)\n", NULL);
  else
    printf("DCAM_IDSTR_CAMERAVERSION  = %s\n", buf);

  if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_DRIVERVERSION, buf,
                         sizeof(buf)))
    dcamcon_show_dcamerr(hdcam, err,
  ...
  if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_MODULEVERSION, buf,
                         sizeof(buf)))
    dcamcon_show_dcamerr(hdcam, err,
  ...
  if (!my_dcamdev_string(err, hdcam, DCAM_IDSTR_DCAMAPIVERSION, buf,
                         sizeof(buf)))
    dcamcon_show_dcamerr(
        hdcam, err, "dcamdev_getstring(DCAM_IDSTR_DCAMAPIVERSION)\n", NULL);
  else
...
HDCAM dcamcon_init_open() {
  ...
  if (nDevice > 1) {
    ...
    printf("choose one of camera from above list by index (0-%d) >",
           nDevice - 1);

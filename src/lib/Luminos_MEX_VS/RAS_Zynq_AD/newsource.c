 ...
 *  and UDP/IP communication. Typically, this driver is used to support
 * host-target communication where the client-side device driver runs on the
 * host and the server-side driver runs on the target. For this implementation,
 * both client-side and server-side driver code has been combined into a single
 *  file.
 *
...
#define SERVER_PORT_PRINTF(FORMAT, ARG1)                                       \
...
typedef struct ServerData_tag {
  ...
  char *
      serverInfoFile; /* the filename that is used to write the server
...
typedef struct ConnectionData_tag {
  ...
  int blockingRecvTimeout;  /*    Timeout value in seconds. rtIOStreamRecv
                                  ...
                                  means to block for up to 10 ms to avoid high
                               CPU  load.
                             */
  VerbosityLevel verbosity; /* enum indicating the level of verbosity to be
 ...
 * send / recv buffers are dynamically allocated and freed when the
 * ConnectionData actually becomes in use.
 *
...
static int initConnectionData(int connectionID, int isServer,
                              CommsProtocol protocol, SOCKET sock,
                              int blockingRecvTimeout, int maxPacketSize,
                              int serverPort, char *serverInfoFile,
                              VerbosityLevel verbosity, int isUsingSeqNum,
                              int udpSendBufSize, int udpRecvBufSize);
...
static int socketDataSet(ConnectionData *connection, const void *src,
                         const size_t size, size_t *sizeSent);

static int socketDataGet(ConnectionData *connection, char *dst,
                         const size_t size, size_t *sizeRecvd);

static int socketDataPending(const SOCKET sock, ConnectionData *connection,
                             int *outPending, int timeoutSecs);

static int serverStreamRecv(ConnectionData *connection, void *dst, size_t size,
                            size_t *sizeRecvd);

static SOCKET serverOpenSocket(int port, char *serverInfoFile,
                               CommsProtocol protocol, int udpSendBufSize,
                               int udpRecvBufSize);
...
static int processArgs(const int argc, void *argv[], char **hostName,
                       unsigned int *portNum, unsigned int *isClient,
                       int *isBlocking, int *recvTimeout, char **serverInfoFile,
                       CommsProtocol *protocol, int *maxPacketSize,
                       VerbosityLevel *verbosity, int *isUsingSeqNum,
                       int *udpSendBufSize, int *udpRecvBufSize);
 ...
 * ================================================= Abstract: Initializes a
 * client / server ConnectionData for the specified protocol.
 *
 ...
 * ================================================= Abstract: Frees memory
 * associated with the ConnectionData referenced by connectionID. Marks the
 * ConnectionData instance as no longer in use.
 */
static void freeConnectionData(ConnectionData *connection) {
 ...
 * ================================================= Abstract: Allocates storage
 * for and initializes a UDPPacketBuffer
 */
static UDPPacketBuffer *createUDPPacketBuffer(int maxPacketSize) {
  /* initialize the UDP packet buffer */
  UDPPacketBuffer *udpPacketBuffer =
      (UDPPacketBuffer *)malloc(sizeof(UDPPacketBuffer));
 ...
 * ================================================= Abstract: Frees memory
 * associated with the referenced UDPPacketBuffer
 */
static void freeUDPPacketBuffer(UDPPacketBuffer **udpPacketBuffer) {
  if (*udpPacketBuffer != NULL) {
    /* free the buffer */
 ...
 * ================================================= Abstract: Resets the
 * referenced UDP Packet Buffer so that it is ready to receive fresh data
 */
static void resetUDPPacketBuffer(UDPPacketBuffer *udpPacketBuffer) {
...
#pragma warning(pop)
  ...
  /* On Linux an extra check of POLLERR flag is needed to detect and report
     an error if the other end suddenly crashes. The same flag doesn't seem to
     be activated on Mac */
  if ((pending == SOCK_ERR) || (PollReadFd[0].revents & POLLERR))
#endif
  {
    retVal = RTIOSTREAM_ERROR;
  }
 ...
 * ===================================================== Abstract: Reads data
 * from the client via "recvfrom" into the UDP packet buffer. The client
 * sockaddr is returned via clientSA and clientSALen.
 *
...
static int initialUDPServerRecvfrom(ConnectionData *connection,
  ...
  *clientSALen = sizeof(*clientSA);
  ...
  nRead = recvfrom(connection->serverData->listenSock, udpPacketBuffer->dataPtr,
                   (size_t)connection->udpData->maxPacketSize, 0U, clientSA,
                   clientSALen);
 ...
 * ===================================================== Abstract: Processes
 * sequence numbers in received UDP datagrams.
 *
 *  RTIOSTREAM_NO_ERROR is returned on success, RTIOSTREAM_ERROR is returned on
 *  failure.
 */
static int processUDPRecvSeqNum(ConnectionData *connection) {
  ...
  if (connection->udpData->isUsingSeqNum) {
     ...
     * host Endian */
    memcpy(&recvSeqNum, udpPacketBuffer->dataPtr, UDP_SEQ_NUM_SIZE);
    ...
    if (connection->udpData->resetExpectedRecvSeqNum) {
      ...
      if (recvSeqNum != connection->udpData->expectedRecvSeqNum) {
        printf("UDP packet sequence number mismatch. Expected #: %u, Actual #: "
               "%u\n",
...
static int socketDataGet(ConnectionData *connection, char *dst,
  ...
  if (connection->protocol == TCP_PROTOCOL) {
    ...
    if (udpPacketBuffer->dataAvail == 0) {
      ...
      resetUDPPacketBuffer(udpPacketBuffer);
      /* read into buffer */
      nRead = recv(connection->sock, udpPacketBuffer->dataPtr,
                   (size_t)connection->udpData->maxPacketSize, 0U);
...
static int socketDataSet(ConnectionData *connection, const void *src,
  ...
  if (connection->protocol == UDP_PROTOCOL) {
    ...
    if (connection->udpData->isUsingSeqNum) {
       ...
       * sequence number is always transmitted / received in
       * host Endian */
      memcpy(udpPacketBuffer->dataPtr, &connection->udpData->sendSeqNum,
             UDP_SEQ_NUM_SIZE);
      udpPacketBuffer->dataPtr += UDP_SEQ_NUM_SIZE;
      udpPacketBuffer->dataAvail += UDP_SEQ_NUM_SIZE;
      /* copy the data - don't overflow the packet buffer */
      transferAmount = MIN(sizeLim, maxPacketSize - udpPacketBuffer->dataAvail);
      memcpy(udpPacketBuffer->dataPtr, src, (size_t)transferAmount);
  ...
  if (nSent == SOCK_ERR) {
    ...
    if ((connection->protocol == UDP_PROTOCOL) &&
        (connection->udpData->isUsingSeqNum) && (nSent > 0)) {
  ...
  *sizeRecvd = 0;
  ...
  if (connection->sock != INVALID_SOCKET) {
    ...
    if (connection->blockingRecvTimeout != BLOCKING_RECV_TIMEOUT_NEVER) {
      ...
      retVal = socketDataPending(connection->sock, connection, &pending,
                                 connection->blockingRecvTimeout);
    } else {
...
static SOCKET serverOpenSocket(int port, char *serverInfoFile,
  ...
  int sockStatus;
  rtiostream_socklen_t sFdAddSize =
      (rtiostream_socklen_t)sizeof(struct sockaddr_in);
  SOCKET lFd;
  int option;
  ...
  if (lFd == INVALID_SOCKET) {
     ...
     */
    option = 1;
    sockStatus = setsockopt(lFd, SOL_SOCKET, SO_REUSEADDR, (char *)&option,
                            sizeof(option));
    ...
    if (protocol == TCP_PROTOCOL) {
      if (lFd != INVALID_SOCKET) {
        ...
        option = 1;
        sockStatus = setsockopt(lFd, IPPROTO_TCP, TCP_NODELAY, (char *)&option,
                                sizeof(option));
        if (sockStatus == SOCK_ERR) {
    ...
    if (protocol == UDP_PROTOCOL) {
      if (lFd != INVALID_SOCKET) {
         ...
         * transfer rate */
        sockStatus = setsockopt(lFd, SOL_SOCKET, SO_SNDBUF,
                                (char *)&udpSendBufSize, sizeof(int));
        if (sockStatus == SOCK_ERR) {
      ...
      if (lFd != INVALID_SOCKET) {
        /* increase the UDP socket receive size to decrease the
         * possibility of buffer overflow */
        sockStatus = setsockopt(lFd, SOL_SOCKET, SO_RCVBUF,
                                (char *)&udpRecvBufSize, sizeof(int));
        if (sockStatus == SOCK_ERR) {
  ...
  if (lFd != INVALID_SOCKET) {
    if (port == 0) {
      ...
      sockStatus =
          getsockname(lFd, (struct sockaddr *)&serverAddr, &sFdAddSize);
      if (sockStatus == SOCK_ERR) {
        ...
        if (serverInfoFile != NULL) {
...
#ifdef __LCC64__
          /* This is needed due to an issue with LCC64, see the following geck:
           * g919889 */
          fseek(fh, 0, SEEK_END);
#endif
          if (fh == NULL) {
          ...
          if (fprintfErr < 0) {
            fprintf(stderr,
                    "Server port number is: %d\nUnable to write server port "
                    "number to output file: %s\n",
          ...
          if (fcloseErr == EOF) {
            fprintf(stderr,
                    ...
                    "after writing server port number: %s\n",
                    serverPortNo, strerror(errno));
            lFd = INVALID_SOCKET;
          }
...
static void serverAcceptSocket(ConnectionData *connection) {
  ...
  if (connection->serverData->listenSock == INVALID_SOCKET) {
    connection->serverData->listenSock = serverOpenSocket(
        connection->serverData->port, connection->serverData->serverInfoFile,
        connection->protocol, connection->udpSendBufSize,
        connection->udpRecvBufSize);
  }

  /* pass listenSock rather than sock */
  error = socketDataPending(connection->serverData->listenSock, connection,
                            &pending, connection->blockingRecvTimeout);
  ...
  if ((pending > 0) && (error == RTIOSTREAM_NO_ERROR)) {
    if (connection->protocol == TCP_PROTOCOL) {
      ...
      cFd = accept(connection->serverData->listenSock,
                   (struct sockaddr *)&clientAddr, &sFdAddSize);
      ...
      if (error == RTIOSTREAM_ERROR) {
        ...
        if (connect(connection->serverData->listenSock, &clientSA,
...
static int processArgs(const int argc, void *argv[], char **hostName,
  ...
  while (count < argc) {
    ...
    if (option != NULL) {
      ...
      if ((strcmp(option, "-hostname") == 0) && (count != argc)) {
        ...
        *isBlocking = (strcmp((char *)argv[count], "1") == 0);
      ...
      } else if ((strcmp(option, "-recv_timeout_secs") == 0) &&
                 (count != argc)) {
        char tmpstr[2];
        int itemsConverted;
        ...
        itemsConverted =
            sscanf(timeoutSecsStr, "%d%1s", (int *)recvTimeout, tmpstr);
        if (itemsConverted != 1) {
          retVal = RTIOSTREAM_ERROR;
        } else {
      ...
      } else if ((strcmp(option, "-server_info_file") == 0) &&
                 (count != argc)) {
        ...
        *isUsingSeqNum = 0;
        ...
        } else if (strcmp(protocolStr,
                          UDP_PACKET_LOSS_DETECTON_PROTOCOL_STRING) == 0) {
          *protocol = UDP_PROTOCOL;
      ...
      } else if ((strcmp(option, "-udpmaxpacketsize") == 0) &&
                 (count != argc)) {
        char tmpstr[2];
        int itemsConverted;
      ...
      } else if ((strcmp(option, "-udpsendbuffersize") == 0) &&
                 (count != argc)) {
        char tmpstr[2];
        int itemsConverted;
        ...
        itemsConverted =
            sscanf(udpSendBufSizeStr, "%d%1s", udpSendBufSize, tmpstr);
        if (itemsConverted != 1) {
          retVal = RTIOSTREAM_ERROR;
        } else {
          argv[count - 2] = NULL;
      ...
      } else if ((strcmp(option, "-udpreceivebuffersize") == 0) &&
                 (count != argc)) {
        char tmpstr[2];
        int itemsConverted;
        ...
        itemsConverted =
            sscanf(udpRecvBufSizeStr, "%d%1s", udpRecvBufSize, tmpstr);
        if (itemsConverted != 1) {
          retVal = RTIOSTREAM_ERROR;
        } else {
          argv[count - 2] = NULL;
...
static SOCKET clientOpenSocket(char *hostName, unsigned int portNum,
  ...
  if (protocol == UDP_PROTOCOL) {
    if (errStatus != RTIOSTREAM_ERROR) {
      /* increase the UDP socket send size to increase the
       * transfer rate */
      int sockStatus = setsockopt(cSock, SOL_SOCKET, SO_SNDBUF,
                                  (char *)&udpSendBufSize, sizeof(int));
      if (sockStatus == SOCK_ERR) {
        printf("setsocketopt() SO_SNDBUF call failed.\n");
    ...
    if (errStatus != RTIOSTREAM_ERROR) {
      /* increase the UDP socket receive size to decrease the
       * possibility of buffer overflow */
      int sockStatus = setsockopt(cSock, SOL_SOCKET, SO_RCVBUF,
                                  (char *)&udpRecvBufSize, sizeof(int));
      if (sockStatus == SOCK_ERR) {
        printf("setsocketopt() SO_RCVBUF call failed.\n");
        cSock = INVALID_SOCKET;
 ...
 * closing its socket without error) or that it timed out waiting for the client
 * to close its socket.
 *
 */
static int waitForClientClose(ConnectionData *connection) {
...
#define TMP_BUF_SIZE (40)
  ...
  do {
    retVal =
        serverStreamRecv(connection, (void *)tmpBuf, TMP_BUF_SIZE, &numRecvd);
...
int rtIOStreamOpen(int argc, void *argv[]) {
  ...
  result = processArgs(argc, argv, &xHostName, &xPortNum, &isClient,
                       &isBlockingRecv, &blockingRecvTimeout, &serverInfoFile,
                       &protocol, &maxPacketSize, &verbosity, &isUsingSeqNum,
  ...
  if (isBlockingRecv) {
    /* blocking: if blockingRecvTimeout has not been set, initialize to the
     * client or server specific default */
  ...
  if (result != RTIOSTREAM_ERROR) { /* polyspace DEFECT:USELESS_IF [Not a
                                       defect:Unset] "Needed for Windows" */
    if (isClient == 1) {
  ...
  if (result != RTIOSTREAM_ERROR) {
    ...
    result = initConnectionData(streamID, isServer, protocol, sock,
                                blockingRecvTimeout, maxPacketSize,
                                (int)xPortNum, serverInfoFile, verbosity,
                                isUsingSeqNum, udpSendBufSize, udpRecvBufSize);
  }
 ...
 * ===================================================== Abstract: Sends the
 * specified number of bytes on the comm line. Returns the number of bytes sent
 * (if successful) or a negative value if an error occurred. As long as an error
 * does not occur, this function is guaranteed to set the requested number of
 * bytes; the function blocks if tcpip's send buffer doesn't have room for all
 * of the data to be sent
 */
int rtIOStreamSend(int streamID, const void *src, size_t size,
                   size_t *sizeSent) {
  int retVal = RTIOSTREAM_NO_ERROR;
  ...
  if (connection->verbosity) {
    if ((*sizeSent > 0) || (connection->verbosity >= VERBOSITY_LEVEL_2)) {
      ...
      printf("rtIOStreamSend (connection id %d): size = %lu, sizeSent = %lu: ",
             streamID, (unsigned long)size, (unsigned long)*sizeSent);

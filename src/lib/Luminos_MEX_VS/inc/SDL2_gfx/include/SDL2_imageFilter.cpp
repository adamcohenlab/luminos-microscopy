...
/*!
\brief Static state which enables the use of the MMX routines. Enabled by
default
*/
...
/*!
\brief Disable MMX check for filter functions and and force to use non-MMX C
based code.
*/
...
L1010:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    *mDest = _m_paddusb(*mSrc1,
                        *mSrc2); /* Src1+Src2 (add 8 bytes with saturation) */
    mSrc1++;
    mSrc2++;
    mDest++;
  }
...
int SDL_imageFilterMean(unsigned char *Src1, unsigned char *Src2,
                        unsigned char *Dest, unsigned int length) {
  static unsigned char Mask[8] = {0x7F, 0x7F, 0x7F, 0x7F,
                                  0x7F, 0x7F, 0x7F, 0x7F};
  unsigned int i, istart;
  unsigned char *cursrc1, *cursrc2, *curdst;
  int result;
...
L1012:
  ...
  int i;
  for (i = 0; i < SrcLength / 8; i++) {
    *mDest = _m_psubusb(*mSrc1,
                        *mSrc2); /* Src1-Src2 (sub 8 bytes with saturation) */
    mSrc1++;
    mSrc2++;
    mDest++;
  }
...
L1013:
  ...
  int i;
  for (i = 0; i < SrcLength / 8; i++) {
    __m64 mm1 = _m_psubusb(
        *mSrc2, *mSrc1); /* Src1-Src2 (sub 8 bytes with saturation) */
    __m64 mm2 = _m_psubusb(
        *mSrc1, *mSrc2);       /* Src2-Src1 (sub 8 bytes with saturation) */
...
L1014:
   ...
   * *\/ */
  /* 	"pxor      %%mm5, %%mm1 \n\t"	/\* take 1's compliment of only neg.
   * words *\/ */
  /* 	"pxor      %%mm6, %%mm2 \n\t"	/\* take 1's compliment of only neg.
   * words *\/ */
  /* 	"psubsw    %%mm5, %%mm1 \n\t"	/\* add 1 to only neg. words, W-(-1) or
   * W-0 *\/ */
  /* 	"psubsw    %%mm6, %%mm2 \n\t"	/\* add 1 to only neg. words, W-(-1) or
   * W-0 *\/ */
  /* 	"packuswb  %%mm2, %%mm1 \n\t"	/\* pack words back into bytes with
   * saturation *\/ */
   ...
   * *\/ */
  /* 	"emms          \n\t"	/\* exit MMX state *\/ */
  /* 	: "+a" (Src1),		/\* load Src1 address into rax, modified by the
   * loop *\/ */
  /* 	  "+b" (Src2),		/\* load Src2 address into rbx, modified by the
   * loop *\/ */
  /* 	  "+c" (SrcLength),	/\* load loop counter (SIZE) into rcx, modified
   * by the loop *\/ */
  /* 	  "+D" (Dest)		/\* load Dest address into rdi, modified by the
   * loop *\/ */
  /* 	: */
  /* 	: "memory",		/\* *Dest is modified *\/ */
  /*           "mm0","mm1","mm2","mm3","mm4","mm5","mm6"	/\* registers
   * modified *\/ */
  /* ); */
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    ...
    *mDest =
        _m_packuswb(mm1, mm2); /* pack words back into bytes with saturation */
    mSrc1++;
    mSrc2++;
    mDest++;
  }
...
/*!
\brief Internal MMX Filter using AddUint: D = saturation255((S[i] + Cs[i % 4]),
Cs=Swap32((uint)C)
...
static int SDL_imageFilterAddUintMMX(unsigned char *Src1, unsigned char *Dest,
                                     unsigned int SrcLength, unsigned int C,
                                     unsigned int D) {
#ifdef USE_MMX
#if !defined(GCC__)
  __asm
...
/*!
\brief Filter using AddUint: D = saturation255((S[i] + Cs[i % 4]),
Cs=Swap32((uint)C)
...
L1022:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    __m64 mm2 =
        _m_psrlwi(*mSrc1, 1);      /* shift 4 WORDS of MM2 1 bit to the right */
    mm2 = _m_pand(mm2, *mMask);    /* apply Mask to 8 BYTES of MM2 */
                                   /* byte     0x0f, 0xdb, 0xd0 */
...
/*!
\brief Internal MMX Filter using SubUint: D = saturation0(S[i] - Cs[i % 4]),
Cs=Swap32((uint)C)
...
/*!
\brief Filter using SubUint: D = saturation0(S[i] - Cs[i % 4]),
Cs=Swap32((uint)C)
...
L10241:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    __m64 mm0 =
        _m_psrlwi(*mSrc1, N);   /* shift 4 WORDS of MM0 (N) bits to the right */
...
/*!
\brief Internal MMX Filter using ShiftRightUint: D = saturation0((uint)S[i] >>
N)
...
L10252:
  ...
  if (C <= 128) { /* if (C <= 128) execute more efficient code */
    for (i = 0; i < SrcLength / 8; i++) {
      __m64 mm3, mm4;
      mm3 =
          _m_punpcklbw(*mSrc1, mm0); /* unpack low  bytes of Src1 into words */
      mm4 =
          _m_punpckhbw(*mSrc1, mm0); /* unpack high bytes of Src1 into words */
      mm3 = _m_pmullw(mm3, mm1);     /* mul low  bytes of Src1 and MM1 */
      mm4 = _m_pmullw(mm4, mm1);     /* mul high bytes of Src1 and MM1 */
      *mDest = _m_packuswb(
          mm3, mm4); /* pack words back into bytes with saturation */
      mSrc1++;
      mDest++;
    }
  } else {
...
/*!
\brief Internal MMX Filter using ShiftRightAndMultByByteMMX: D =
saturation255((S >> N) * C)
...
L1026:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    ...
    mm4 = _m_pmullw(mm4, mm1); /* mul high bytes of Src1 and MM1 */
    *mDest =
        _m_packuswb(mm3, mm4); /* pack words back into bytes with saturation */
    mSrc1++;
    mDest++;
  }
  _m_empty(); /* clean MMX state */
...
L10271:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    __m64 mm0 =
        _m_psllwi(*mSrc1, N);   /* shift 4 WORDS of MM0 (N) bits to the left */
    *mDest = _m_pand(mm0, mm1); /* apply proper bit-Mask to 8 BYTES of MM0 */
    mSrc1++;
    mDest++;
  }
...
int SDL_imageFilterShiftLeftByte(unsigned char *Src1, unsigned char *Dest,
                                 unsigned int length, unsigned char N) {
  static unsigned char Mask[8] = {0xFE, 0xFE, 0xFE, 0xFE,
                                  0xFE, 0xFE, 0xFE, 0xFE};
  unsigned int i, istart;
  unsigned char *cursrc1, *curdest;
  int result;
...
L1029:
  ...
  for (i = 0; i < SrcLength / 8; i++) {
    __m64 mm0 = _m_paddusb(
        *mSrc1, mm2); /* Src1+(0xFF-T) (add 8 bytes with saturation) */
...
/*!
\brief Internal MMX Filter using ClipToRange: D = (S >= Tmin) & (S <= Tmax)
S:Tmin | Tmax
...
/*!
\brief Internal MMX Filter using NormalizeLinear: D = saturation255((Nmax -
Nmin)/(Cmax - Cmin)*(S - Cmin) + Nmin)
...
static int SDL_imageFilterNormalizeLinearMMX(unsigned char *Src1,
                                             unsigned char *Dest,
                                             unsigned int SrcLength, int Cmin,
...
/*!
\brief Filter using NormalizeLinear: D = saturation255((Nmax - Nmin)/(Cmax -
Cmin)*(S - Cmin) + Nmin)
...
int SDL_imageFilterNormalizeLinear(unsigned char *Src, unsigned char *Dest,
                                   unsigned int length, int Cmin, int Cmax,
                                   int Nmin, int Nmax) {
  unsigned int i, istart;
...
\param Src The source 2D byte array to convolve. Should be different from
destination. \param Dest The destination 2D byte array to store the result in.
Should be different from source. \param rows Number of rows in
source/destination array. Must be >2. \param columns Number of columns in
source/destination array. Must be >2. \param Kernel The 2D convolution kernel of
size 3x3. \param Divisor The divisor of the convolution sum. Must be >0.
...
L10322:
    ...
    asm volatile(
        ...
        "pmullw    %%mm7, %%mm3 \n\t" /* multiply words third row  image*Kernel
                                       */
        "paddsw    %%mm2, %%mm1 \n\t" /* add 4 words of the first and second
                                         rows */
        ...
        "jnz            .L10322 \n\t" /* check loop termination, proceed if
                                         required */
...
destination. \param Dest The destination 2D byte array to store the result in.
Should be different from source. \param rows Number of rows in
source/destination array. Must be >4. \param columns Number of columns in
source/destination array. Must be >4. \param Kernel The 2D convolution kernel of
size 5x5. \param Divisor The divisor of the convolution sum. Must be >0.
...
destination. \param Dest The destination 2D byte array to store the result in.
Should be different from source. \param rows Number of rows in
source/destination array. Must be >6. \param columns Number of columns in
source/destination array. Must be >6. \param Kernel The 2D convolution kernel of
size 7x7. \param Divisor The divisor of the convolution sum. Must be >0.
...
destination. \param Dest The destination 2D byte array to store the result in.
Should be different from source. \param rows Number of rows in
source/destination array. Must be >8. \param columns Number of columns in
source/destination array. Must be >8. \param Kernel The 2D convolution kernel of
size 9x9. \param Divisor The divisor of the convolution sum. Must be >0.
...
source/destination array. Must be >2. \param columns Number of columns in
source/destination array. Must be >2. \param Kernel The 2D convolution kernel of
size 3x3. \param NRightShift The number of right bit shifts to apply to the
convolution sum. Must be <7.
...
L10362:
    ...
    asm volatile(
        ...
        "punpcklbw %%mm0, %%mm2 \n\t" /* unpack first 4 bytes into words */
        "punpcklbw %%mm0, %%mm3 \n\t" /* unpack first 4 bytes into words */
        "psrlw     %%mm4, %%mm1 \n\t" /* shift right each pixel NshiftRight
                                         times */
        "psrlw     %%mm4, %%mm2 \n\t" /* shift right each pixel NshiftRight
                                         times */
        "psrlw     %%mm4, %%mm3 \n\t" /* shift right each pixel NshiftRight
...
source/destination array. Must be >4. \param columns Number of columns in
source/destination array. Must be >4. \param Kernel The 2D convolution kernel of
size 5x5. \param NRightShift The number of right bit shifts to apply to the
convolution sum. Must be <7.
...
source/destination array. Must be >6. \param columns Number of columns in
source/destination array. Must be >6. \param Kernel The 2D convolution kernel of
size 7x7. \param NRightShift The number of right bit shifts to apply to the
convolution sum. Must be <7.
...
source/destination array. Must be >8. \param columns Number of columns in
source/destination array. Must be >8. \param Kernel The 2D convolution kernel of
size 9x9. \param NRightShift The number of right bit shifts to apply to the
convolution sum. Must be <7.
...
\param Src The source 2D byte array to sobel-filter. Should be different from
destination. \param Dest The destination 2D byte array to store the result in.
Should be different from source. \param rows Number of rows in
source/destination array. Must be >2. \param columns Number of columns in
source/destination array. Must be >7.
...
L10402:
    ...
    asm volatile(
        ...
        "psrlq       $32, %%mm6 \n\t" /* shift 2 left words to the right */
        "psubw     %%mm3, %%mm6 \n\t" /* MM6 = MM6 - MM3 */
        "punpckldq %%mm6, %%mm4 \n\t" /* combine 2 words of MM6 and 2 words of
                                         MM4 */
        "movq      %%mm5, %%mm2 \n\t" /* copy MM6 into MM2 */
        "psrlq       $32, %%mm5 \n\t" /* shift 2 left words to the right */
        "psubw     %%mm2, %%mm5 \n\t" /* MM5 = MM5 - MM2 */
        ...
        "psrlq       $32, %%mm7 \n\t" /* shift 2 left words to the right */
        "psubw     %%mm3, %%mm7 \n\t" /* MM7 = MM7 - MM3 */
        "punpckldq %%mm7, %%mm5 \n\t" /* combine 2 words of MM7 and 2 words of
                                         MM5 */
        /* Take abs values of MM4 and MM5 */
        "movq      %%mm4, %%mm6 \n\t" /* copy MM4 into MM6 */
        "movq      %%mm5, %%mm7 \n\t" /* copy MM5 into MM7 */
        ...
        "jnz            .L10402 \n\t" /* check loop termination, proceed if
                                         required */
        "mov       %%ebx, %%esi \n\t" /* restore most left current row Src
                                         address */
        "movd      %%mm1, %%edi \n\t" /* restore most left current row Dest
                                         address */
        "add       %%eax, %%esi \n\t" /* move to the next row in Src */
        "add       %%eax, %%edi \n\t" /* move to the next row in Dest */
        "dec              %%edx \n\t" /* decrease loop counter ROWS */
...
Should be different from source. \param rows Number of rows in
source/destination array. Must be >2. \param columns Number of columns in
source/destination array. Must be >8. \param NRightShift The number of right bit
shifts to apply to the filter sum. Must be <7.
...
void SDL_imageFilterAlignStack(void) {
  ...
  asm volatile(                              /* --- stack alignment --- */
               "mov       %%esp, %%ebx \n\t" /* load ESP into EBX */
               "sub          $4, %%ebx \n\t" /* reserve space on stack for old
                                                value of ESP */
               "and        $-32, %%ebx \n\t" /* align EBX along a 32 byte
                                                boundary */
               "mov     %%esp, (%%ebx) \n\t" /* save old value of ESP in stack,
                                                behind the bndry */
               "mov       %%ebx, %%esp \n\t" /* align ESP along a 32 byte
                                                boundary */
               ::);
#endif
#endif

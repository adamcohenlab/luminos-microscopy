 ...
 *  \param rect      A pointer to the rectangle to lock for access. If the rect
 *                   is NULL, the entire texture will be locked.
 *  \param surface   This is filled in with a SDL surface representing the
 * locked area Surface is freed internally after calling SDL_UnlockTexture or
 * SDL_DestroyTexture.
 *
 *  \return 0 on success, or -1 if the texture is not valid or was not created
 * with ::SDL_TEXTUREACCESS_STREAMING.
 ...
 *         If SDL_LockTextureToSurface() was called for locking, the SDL surface
 * is freed.
 *
 *  \sa SDL_LockTexture()
